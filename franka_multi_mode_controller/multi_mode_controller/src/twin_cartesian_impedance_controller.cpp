#include <multi_mode_controller/controllers/twin_cartesian_impedance_controller.h>
...
using Controller = TwinCartesianImpedanceController;


static auto registration = ControllerFactory::registerClass<Controller>(
    "twin_cartesian_impedance_controller");
...
bool Controller::desiredPoseCallbackImpl(Pose& p_d, 
  ...
  for (int i=0; i<2; i++) {
    if (!transformWorldPoseToFrankaBase(world_position, world_orientation,
                                        world_to_franka_base_.at(i),
                                        p_d.poses[i].position,
                                        p_d.poses[i].orientation)) {
      ...
      RCLCPP_WARN_THROTTLE(
          logger, clk, 1000,
          "twin_cartesian_impedance_controller: Discarding target pose with invalid orientation quaternion.");
      return false;
    }
  }
  ...
  if ((p_d.poses[0].position+PandaControllerBase<Parameters, Pose>::getOffset().poses[0].position-p.poses[0].position).norm() > 0.1 ||
    ...
    auto& clk = *PandaControllerBase<Parameters, Pose>::node_->get_clock();
    const auto& logger = PandaControllerBase<Parameters, Pose>::node_->get_logger();
    RCLCPP_WARN_THROTTLE(logger, clk, 1000, "twin_cartesian_impedance_controller: Discarding "
