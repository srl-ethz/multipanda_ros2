#include <multi_mode_controller/controllers/comless_coupled_dual_cartesian_impedance_controller.h>
...
void Controller::computeTauImpl(const std::vector<std::array<double, 7>*>& tau,
  ...
  bool isRight = false;
  for(int i = 0; i < 2; i++){
    auto desired = PandaCartesianImpedanceControllerPose();
    auto& current = current_poses.poses[i];
    if(isRight){
      auto& temp = current_poses.poses[0];
      desired.position = Vector3d(temp.position[0], temp.position[1] + 0.22, temp.position[2]);
      desired.orientation = current_poses.poses[0].orientation;
    }
    else{
      desired = desired_poses.poses[0];
      desired.orientation = Quaterniond(0, 0.9238795, 0.3826834, 0);
    }
...
Params Controller::defaultParameters() {
  Params p;
  p.params[0].stiffness.setIdentity();
  p.params[0].stiffness.topLeftCorner(3, 3) << 0 * Eigen::Matrix3d::Identity();
  p.params[0].stiffness(1,1) = 600;
  p.params[0].stiffness.bottomRightCorner(3, 3) << 20 * Eigen::Matrix3d::Identity();
  p.params[0].damping_ratio = Vector6d::Constant(0.8);
  p.params[0].nullspace_stiffness = 0;
  p.params[1].stiffness.setIdentity();
  p.params[1].stiffness.topLeftCorner(3, 3) << 600 * Eigen::Matrix3d::Identity();
  p.params[1].stiffness(0,0) = 1000.0; // Make just x-z axes stiffer
  p.params[1].stiffness(2,2) = 1000.0; // Make just x-z axes stiffer

  p.params[1].stiffness.bottomRightCorner(3, 3) << 20 * Eigen::Matrix3d::Identity();
  p.params[1].damping_ratio = Vector6d::Constant(0.8);
  p.params[1].nullspace_stiffness = 0;
  return p;
}
